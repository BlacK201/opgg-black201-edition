...
const App = () => {
  ...
  const [isTFT, setIsTFT] = useState(false);
  ...
  useEffect(() => {
    ...
    window.api.on("overlay-setting-changed", (event, data) => {
      setOverlaySettings(JSON.parse(data ?? "{}"));
    });
    ...
    <div className={"overlay-main"} style={{
      ...
      <div className={"main-interface"}
           style={{
             ...
             display: `${(isHidden || isTFT || !(overlaySettings["lol-skill"] ?? true)) ? "none" : "block"}`,
        ...
        <div style={{
          ...
          <img src={"https://desktop-app-data.op.gg/overlay-skill.png"}
               style={{
                  position: "absolute",
                  top: "-49px",
                  width: "71px",
      ...
      {(!diffHidden && !isTFT && (overlaySettings["lol-benchmark"] ?? true)) &&
      ...
      <div style={{
        color: "#fff",
        display: `${(isTFT && (overlaySettings["tft-meta"] ?? true)) ? "block" : "none"}`
      }}>
        <Tft mode={"overlay"} />
      </div>
    </div>
