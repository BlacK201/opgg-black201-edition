...
import { timeForToday, copyToClipboard } from "../../utils/utils";
import Tippy from "@tippyjs/react";
import sendGA4Event from "../../utils/ga4";
import NotSupported from "../../components/common/NotSupported";
import {useDispatch} from "react-redux";
import {setGPM, setIsSettingOpen, setRiotAccounts} from "../../redux/slices/common";
import RsoInput from "../components/common/RsoInput";
...
const MyPage = () => {
    ...
    const {summonerName, gpm, riotAccounts} = useTypedSelector((state) => state.common);
    ...
    const editProfile = (feature="") => () => {
        ...
        if (feature === "") {
            sendGA4Event("click", {
                ...
                label_category: "summoner_profile",
                label_name: "edit_profile_button"
            });
        } else if (feature === "fan") {
            tabIndex = 1;
            sendGA4Event("click", {
    ...
    const verifySummoner = () => {
        sendGA4Event("click", {
            ...
            event_result: "copy_url",
            label_category: "summoner_profile",
            label_name: "rso_verify_url_button"
        });

        window.api.invoke("verificationURL").then((response) => {
            copyToClipboard(response);
            customToastr.success(t("rso.verified-url-copied"));
        });
    }
        ...
        window.api.invoke("mypage", localStorage.getItem("currentUser") ?? summonerName ?? "Hide on bush").then((tmp) => {
            if (tmp) {
                if (mounted.current) {
                    ...
                    try {
                        let bla = _.sortBy(_.groupBy(_.filter(tmp, (o) => {
                            return o.queueType === "rank5solo" || o.queueType === "rank5flex";
                        }), "championId"), [(o) => {
                            return -o.length;
                        }]);
                        ...
                        bla.map((b, i) => {
                            if (i < 10) {
                                tmpMostChampion.push(b[0].championId);
                            }
                        })
                        localStorage.setItem("most-champions", JSON.stringify(tmpMostChampion));
                    } catch (_) {}
    ...
    const renewal = _.debounce((showToast=true) => {
        if (!isRenewing) {
            if (showToast) {
    ...
    let backgroundImage = _.find(championsMetaData.data, {
        id: mostChampions.length >= 1 ? mostChampions[0][0].championId : 1
    })?.key;
    backgroundImage = `https://ddragon.leagueoflegends.com/cdn/img/champion/splash/${backgroundImage}_0.jpg`;
    if (gpm?.cover_champion) {
        backgroundImage = _.find(_.find(championsMetaData.data, {
            id: gpm?.cover_champion.id
        })?.skins, {
            id: gpm?.cover_champion.skin_id
        })?.centered_image;
    }
            ...
            <div className={"main-container mypage"}>
                <div className={"mypage-top"}>
                    ...
                    <div className={"mypage-top__top"}>
                        <div className={"mypage-top__top-header"}>
                            <div className={`mypage-update-btn ${isRenewed ? "mypage-update-btn-done" : ""}`}
                                 onClick={() => {
                                     renewal();
                                 }}><img src={"../../assets/images/icon-refresh.svg"} width={16} style={{marginRight: "4px"}}/>
                                {isRenewing
                                    ? <>{t("renewing")}</>
                                    : <>{isRenewed
                                        ? <>{t("renewed")}</>
                            ...
                            {rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO).result && rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO).isCurrentAccount &&
                                <>
                                    <div className={"mypage-summoner-rso-btn"} onClick={verifySummoner}>
                                        <img src={"../../assets/images/icon-outlink-wh.svg"} width={16} style={{marginRight: 4}} />
                                        {t("rso.verified-url")}
                                    </div>
                                    <div className={"mypage-rso-settings-btn"} onClick={editProfile("")}>
                                        <img src={"../../assets/images/icon-edit.svg"} width={16} style={{marginRight: 4}} />
                                        {t("rso.edit-profile")}
                                    </div>
                                </>
                            }
                        ...
                        {allChampions &&
                            <div className={"mypage-recent-data"}>
                                <div className={"mypage-recent-data__item"}>
                                    <div>{t("mypage.win-ratio")}</div>
                                    <div className={"label"}>
                                        {(_.sumBy(_.slice(allChampions, 0, 20), (o: any) => {
                                            return o.stats["CareerStats.js"].victory;
                                        }) / 20 * 100).toFixed(0)}%
                                    </div>
                                </div>
                                <div className={"mypage-recent-data__item"}>
                                    ...
                                    <div className={"label"}>
                                        {getGrade(Math.round(_.sumBy(_.slice(allChampions, 0, 20), (o: any) => {
                                            return o.stats["CareerStats.js"].victory;
                                        }) / 20 * 100))}
                                    </div>
                                </div>
                                <div className={"mypage-recent-data__item"}>
                                    ...
                                    <div className={"label"}>
                                        {getKdaGrade((_.sumBy(allChampions, (o: any) => {
                                            return o.stats["CareerStats.js"].kills + o.stats["CareerStats.js"].assists;
                    ...
                    <div className={"mypage-top__bottom"}>
                        ...
                        {(rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO).result && rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO).isCurrentAccount)
                            ? <>
                                {rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO | constants.TYPE_SET, "fan").result
                                    ? <div className={"mypage-top__bottom-fan"} onClick={editProfile("fan")}>
                                        <img src={`${gpm?.favorite_esport_team?.image_url}?image=w_16,f_webp`} width={16} style={{marginRight: 4}} />
                                        {i18n === "kr"
                                            ? <>{gpm?.favorite_esport_team?.name} 팬입니다.</>
                                            : <>Fan of {gpm?.favorite_esport_team?.name}</>
                                        }
                                    </div>
                                    : <div className={"mypage-top__bottom-fan"} style={{cursor: "pointer"}}
                                           onClick={editProfile("fan")}
                                    >
                                        {t("rso.esports-placeholder")}
                                    </div>
                                }
                            </>
                            : <>
                                {gpm?.favorite_esport_team
                                    && <div className={"mypage-top__bottom-fan"} onClick={editProfile("fan")}>
                                        <img src={`${gpm?.favorite_esport_team?.image_url}?image=w_16,f_webp`} width={16} style={{marginRight: 4}} />
                                        {gpm?.favorite_esport_team?.acronym} 팬입니다.
                                    </div>
                                }
                            </>
                        }
                        ...
                        <div className={"mypage-top__bottom-seasons"}>
                            {mypage.ranked.highestRankedEntrySR.tier !== "NONE"
                                ?
                                <div>S2022 {capitalizeFirstLetter(mypage.ranked.highestRankedEntrySR.tier)} {mypage.ranked.highestRankedEntrySR.division === "NA" ? "" : mypage.ranked.highestRankedEntrySR.division}</div>
                                : <div>S2022 Unranked</div>
                            }
                            {mypage.ranked.highestPreviousSeasonEndTier !== "NONE"
                                ?
                                <div>S2021 {capitalizeFirstLetter(mypage.ranked.highestPreviousSeasonEndTier)} {mypage.ranked.highestPreviousSeasonEndDivision === "NA" ? "" : mypage.ranked.highestPreviousSeasonEndDivision}</div>
                                : <div>S2021 Unranked</div>
                            }
                        </div>
                        <div className={"mypage-top__bottom-profile"}>
                            <div className={"mypage-top__bottom-profile-img"} style={{position: "relative"}}>
                                <img
                                ...
                                {gpm?.is_auth &&
                                    <img src={"../../assets/images/img-rso-certification-mark.webp"} style={{
                                        position: "absolute",
                                        width: 32,
                                        height: 32,
                                        top: -4,
                            ...
                            <div className={"mypage-top__bottom-profile-info"} style={{marginRight: "40px"}}>
                                <div onClick={onClickSummonerName(mypage.summoner.displayName)}
                                     style={{cursor: "pointer", width: "fit-content"}}>{mypage.summoner.displayName} <img
                                    ...
                                    ? <>
                                        {gpm?.word
                                            ? <div className={`introduction`}
                                                   onClick={editProfile("introduce")}
                                            >
                                                {(gpm?.word?.is_blind)
                                                    ? <>
                                                        <img src={"../../assets/images/01-icon-icon-report.svg"} width={16} style={{marginRight: 4}} />
                                                        <span style={{color: "#e84057", wordBreak: "keep-all"}}>{t("rso.report-blind")}</span>
                                                    </>
                                                    : <>
                                                        <img src={"../../assets/images/icon-comment.svg"} width={16} style={{marginRight: 4}} />
                                                        <div style={{wordBreak: "keep-all"}}>{gpm?.word?.content}</div>
                                                    </>
                                                }
                                            </div>
                                            : <div className={"introduction"}
                                                   onClick={editProfile("introduce")}
                                            >
                                                <img src={"../../assets/images/icon-comment-disabled.svg"} width={16} style={{marginRight: 4}} />
                                                {t("rso.comment-placeholder")}
                                            </div>
                                        }
                                    </>
                                    : <>
                                        {gpm?.word
                                            ...
                                            : <>
                                                {!!localStorage.getItem("opgg_nickname")
                                                    ? <>
                                                        {riotAccounts === 0
                                                            && <div className={"introduction"} style={{backgroundColor: "transparent", border: "solid 1px #31313c"}}
                                                                   onClick={editProfile("connect")}
                                                            >
                                                                <img src={"../../assets/images/img-rso-certification-mark.webp"} width={16} style={{marginRight: 4}} />
                                                                {t("rso.rso-connect")}
                                                                <img src={"../../assets/images/icon-arrow-next.svg"} width={16} style={{marginLeft: 8}} />
                                                            </div>
                                                        }
                                ...
                                {/*                    ? <>*/}
                                {/*                        {rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO | constants.TYPE_SET, "introduce").result*/}
                                {/*                            ? <div className={`introduction`}*/}
                                {/*                                   onClick={editProfile("introduce")}*/}
                                {/*                            >*/}
                                {/*                                {(gpm?.word?.is_blind)*/}
                                {/*                                    ? <>*/}
                                {/*                                        <img src={"../../assets/images/01-icon-icon-report.svg"} width={16} style={{marginRight: 4}} />*/}
                                {/*                                        <span style={{color: "#e84057"}}>{t("rso.report-blind")}</span>*/}
                                {/*                                    </>*/}
                                {/*                                    : <>*/}
                                {/*                                        <img src={"../../assets/images/icon-comment.svg"} width={16} style={{marginRight: 4}} />*/}
                                {/*                                        <div>{gpm?.word?.content}</div>*/}
                                {/*                                    </>*/}
                                {/*                                }*/}
                                {/*                            </div>*/}
                                {/*                            : <div className={"introduction"}*/}
                ...
                <div className={"mypage-stats"} style={{width: "100%", paddingBottom: "8px"}}>
                    ...
                    <div className={"mypage-stats-right"}>
                        {mypage.games.map((game, i) => {
                            ...
                            return (
                                <div className={"mypage-stats-right-list-wrapper"} key={i} style={{
                                    height: `${(rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO).result && rsoFeatureType(constants.TYPE_LOGIN | constants.TYPE_RSO).isCurrentAccount) ? "136px" : "96px"}`
                                }}>
                                    <div
                                    ...
                                    <div className={"mypage-stats-right-list"} >
