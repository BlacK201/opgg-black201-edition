...
const Tft = ({mode}) => {
    ...
    const [tftSet, setTftSet] = useState("8");
    ...
    const [onboarding, setOnboarding] = useState((localStorage.getItem("onboarding2") ?? true) !== "false");


    useEffect(() => {
        // window.api.send("tft-join");
        if (!data) {
            axios.get(`https://desktop-app-data.op.gg/tft/${tftSet}/recommend.json?timestamp=${new Date().getTime()}`).then((res) => {
                setData(res.data);
            }).catch(() => {
            });
        }
        ...
        if (mode !== "overlay") {
            ...
            sendGA4Event("view_tft_page_overlay", {
                "menu_name": "full"
            });
        }
    ...
    useEffect(() => {
        ...
        axios.get(`https://desktop-app-data.op.gg/tft/${tftSet}/meta_kr.json?timestamp=${new Date().getTime()}`).then((res) => {
            ...
            res.data?.traits.map((trait: any) => {
                if (trait.isClass) {
                    tmpDropboxContent.push({
                        ...
                        icon: `https://desktop-app-data.op.gg/tft/${tftSet}/traits-wh/${trait.id}.svg`
                    });
    ...
    if (data && meta) {
        ...
        const synergyToolTip = (s, c) => {
            let i18n = localStorage.getItem("i18n");
            let synergy = _.find(meta.traits, {id: s});
            let tmp = _.findIndex(synergy.effects, (e) => {
                return e.maxUnits >= c && e.minUnits <= c;
            });
            let synergyEffect = i18n === "kr" ? synergy.meta[4].split("\n") : synergy.meta[5].split("\n");
            synergyEffect[tmp] = `<font style="color: #a072ff">${synergyEffect[tmp]?.replace("\r", "")}</font>`;
            synergyEffect.unshift(`<div style='white-space: pre-line; color: #676678; line-height: 1.5'>`);
            synergyEffect.push(`</div>`);
            synergyEffect = synergyEffect.join("\n");
            return (
                <div>
                    ...
                    <div dangerouslySetInnerHTML={{__html: synergyEffect}}></div>
                </div>
            )
        }
                                        ...
                                        return (
                                            <>
                                                <img width={32} height={32}
                                                     src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${id}.png`}
        ...
        const championToolTip = (c) => {
            let i18n = localStorage.getItem("i18n");
            let champion = _.find(meta.champions, {id: c.id});
                ...
                <div className={"tft-tooltip"}>
                    <div style={{
                        ...
                        <div className={"tft-tooltip-cost"}>
                            <img src={"https://desktop-app-data.op.gg/assets/images/icon-gold.svg"} style={{marginLeft: 8, marginRight: 2}} /> {c.cost}
                        </div>
                    </div>
                    <div style={{
                        marginBottom: 10
                        ...
                        {trait.map((d, i) => {
                            return (
                                <div style={{
                                    ...
                                    <img width={15} height={15} src={`https://desktop-app-data.op.gg/tft/${tftSet}/traits-wh/${d.id}.svg`} style={{marginRight: 4}} />
                            ...
                            <div>
                                <div style={{
                                    ...
                                    <div style={{
                                        ...
                                        <img width={32} height={32} src={`https://desktop-app-data.op.gg/tft/${tftSet}/skills/${champion.id}.png`} style={{borderRadius: 4}} />
                                    </div>
                                    <div>
                                        <div style={{
                                            color: "#ff9f4a"
                                            ...
                                            {champion.meta[5].split(",").map((d) => {
                                                ...
                                                if (item) {
                                                    return (
                                                        <>
                                                            <img width={32} height={32}
                                                                 src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${item.id}.png`}
                                                                 style={{
                                                                     borderRadius: 4,
                                                                     marginRight: 4
                                                                 }}/>
                ...
                <div className={`tft-container`} style={{
                    ...
                    {data.recommend.map((d:any, i:any) => {
                        ...
                        if ((mode === "overlay" && i === openedIndex) || mode !== "overlay") {
                            // console.log(d);
                            let synergyCount = d.info.synergies.length;
                            if (synergyCount >= 9) {
                                synergyCount = 9;
                            }
                            let currentHexagon = 0;
                            let sortedSynergies = _.sortBy(d.info.synergies, (e) => {
                                return e.id.split("-")[1];
                            }).reverse();
                            d.info.buildboards.final = _.sortBy(d.info.buildboards.final, "cost");
                            let coreChampions = _.filter(d.info.buildboards.final, {core: true});
                            let items: any = [];
                            for (let a = 1; a <= 3; a++) {
                                _.filter(d.info.buildboards.final, (e) => {
                            ...
                            if (selectedSynergy.value !== "all") {
                                let has = _.find(d.info.synergies, (e) => {
                            ...
                            let youtube = _.find(d.info.streamer, {"platform": "youtube"});
                            let twitch = _.find(d.info.streamer, {"platform": "twitch"});
                            // let tag = null;
                            // if (d.info.tag && d.info.tag.split("|").length > 1) {
                            //     tag = localStorage.getItem("i18n") === "kr" ? d.info.tag.split("|")[0] : d.info.tag.split("|")[1];
                            // }
                            // tag = d.info.tag;
                                        ...
                                        <div
                                            ...
                                            <div className={"title-column"}>
                                                ...
                                                <div style={{
                                                    ...
                                                    <div style={{
                                                        ...
                                                        {localStorage.getItem("i18n") === "kr"
                                                            ? <>
                                                                {d.info.tag.kr && d.info.tag.kr.map((t, i) => {
                                                                    return(
                                                                        <div style={{
                                                                            display: "flex",
                                                            ...
                                                            : <>
                                                                {d.info.tag.en && d.info.tag.en.map((t, i) => {
                                                                    return(
                                                                        <div style={{
                                                                            display: "flex",
                                                                            justifyContent: "center",
                                            ...
                                            <div className={"hexagons"}>
                                                {Array.from(Array(synergyCount).keys()).map((i) => {
                                                    ...
                                                    let count = sortedSynergies[i]?.count ?? 0;
                                                    let style = synergy[1];
                                                    synergy = synergy[0];
                                                    return (
                                                        <div
                                                            className={`hexagon hexagon-${currentHexagon} hexagon-style-${style}`}>
                                                            <Tippy content={synergyToolTip(synergy, count)} placement={"bottom"}>
                                                                <img src={`https://desktop-app-data.op.gg/tft/${tftSet}/traits/${synergy}.svg`}/>
                                                            </Tippy>
                                                        </div>
                                                    )
                                                })}
                                            ...
                                            <div className={"tft-champions"}>
                                                {d.info.buildboards.final.map((c: any, i: any) => {
                                                    if (c.name !== "비취 조각상") {
                                                        return (
                                                            <div className={"tft-champion"}>
                                                                {c?.three &&
                                                                    <div style={{
                                                                        ...
                                                                        <img
                                                                            src={`../../assets/images/star${c.cost}.svg`}/>
                                                                    </div>
                                                                }
                                                                <Tippy content={championToolTip(c)}
                                                                    ...
                                                                    <div className={`tft-cost tft-cost-${c.cost}`}>
                                                                        <img
                                                                            src={`https://desktop-app-data.op.gg/tft/${tftSet}/champions/${c.id}.png`}/>
                                                                        <div
                                                                            className={`tft-cost-area tft-cost-area-${c.cost} `}>${c.cost}</div>
                                                                    </div>
                                                                </Tippy>
                                                                <div className={"tft-items"}>
                                                                    {(c.item1.id && c.item1.id !== 0 && c.item1.id !== null) &&
                                                                        <Tippy content={itemToolTip(c.item1)}
                                                                            ...
                                                                            <div className={"tft-item"}
                                                                                ...
                                                                                <img
                                                                                    src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${c.item1.id}.png`}/>
                                                                            </div>
                                                                        </Tippy>
                                                                    }
                                                                    {(c.item2.id && c.item2.id !== 0 && c.item2.id !== null) &&
                                                                        <Tippy content={itemToolTip(c.item2)}
                                                                            ...
                                                                            <div className={"tft-item"}
                                                                                ...
                                                                                <img
                                                                                    src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${c.item2.id}.png`}/>
                                                                            </div>
                                                                        </Tippy>
                                                                    }
                                                                    {(c.item3.id && c.item3.id !== 0 && c.item3.id !== null) &&
                                                                        <Tippy content={itemToolTip(c.item3)}
                                                                            ...
                                                                            <div className={"tft-item"}
                                                                                ...
                                                                                <img
                                                                                    src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${c.item3.id}.png`}/>
                                                                            </div>
                                                                        </Tippy>
                                                                    }
                                                                </div>
                                                    ...
                                                    ? <div>
                                                        ...
                                                        <div className={"tft-core-champions"}>
                                                            {coreChampions.map((c) => {
                                                                ...
                                                                //     return l.key.toLowerCase() === c.id;
                                                                // })?.id}`);

                                                                return (
                                                                    <>
                                                                        <div
                                                                            className={`tft-core-champion tft-core-champion-${c.cost}`}>
                                                                            <Tippy content={championToolTip(c)}>
                                                                                <img
                                                                                    src={`https://desktop-app-data.op.gg/tft/${tftSet}/champions/${c.id}.png`}
                                                ...
                                                {d?.info.levels &&
                                                    <div style={{
                                                        ...
                                                        <div style={{
                                                            ...
                                                            {Object.keys(d.info.levels).map((level, i) => {
                                                                if (d.info.levels[level] !== "") {
                                                                    return (
                                                                        <>
                                                                            <div style={{
                                                                                display: "flex",
                                                                                ...
                                                                                <div>{d.info.levels[level]}</div>
                                                                            </div>
                                                                            {Object.keys(d.info.levels).length - 1 !== i &&
                                                                                <div>
                                                                                    <img
                                            ...
                                            <div style={{display: "flex", marginTop: "24px"}}>
                                                            ...
                                                            return (
                                                                <div className={"tft-item-combination-duplicate"}>
                                                                    <div className={`tft-item-combination-0`}>
                                                                        <img
                                                                            src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${k}.png`}/>
                                                                    </div>
                                                                    <div>
                                                                        <img
                                                                            src={"../../assets/images/tft/tft-item-plus.svg"}
                                                                    ...
                                                                    <div>
                                                                        {items.from.map((item) => {
                                                                            return (
                                                                                <img
                                                                                    src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${item}.png`}
                                                                                    style={{margin: "0 4px"}}/>
                                                                            )
                                                                        })}
                                                                    </div>
                                                        ...
                                                        {newItems.unique.map((item: any) => {
                                                            return (
                                                                <div className={"tft-item-combination-row"}>
                                                                    <div className={`tft-item-combination-0`}>
                                                                        <img
                                                                            src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${item.from[0]}.png`}/>
                                                                    </div>
                                                                    <div>
                                                                        <img
                                                                            src={"../../assets/images/tft/tft-item-plus.svg"}
                                                                    ...
                                                                    <div>
                                                                        <img
                                                                            src={`https://desktop-app-data.op.gg/tft/${tftSet}/items/${item.from[1]}.png`}/>
                                                                    </div>
                                                                    <div>
                                                                        <img
                                                                            src={"../../assets/images/tft/tft-item-equal.svg"}
                                                ...
                                                {d.info.youtube.link &&
                                                    <div style={{
                                                        ...
                                                        <a href={d.info.youtube.link} target={"_blank"} onClick={() => {
                                                            sendGA4Event("click_tft_youtube_video", {
                                                                title: d.title.kr,
                                                                // tier: guideInfo.tier,
                                                                channel: youtube?.channelUrl,
                                                                channel_name: youtube?.channelName,
                                                                video_url: d.info.youtube.link
                                                            });
                                                        }}>
                                                            <div className={`tft-guide-modal-layer-info-youtube-video ${onboarding ? "onboarding" : ""}`}
                                                                 style={{
                                                                     width: "100%",
                                                                     height: "100%",
                                                                     borderRadius: 4
                                                                 ...
                                                                 onMouseEnter={() => {
                                                                     setOnboarding(false);
                                                                    localStorage.setItem("onboarding2", "false");
                                                                 }}
                                                            >
                                                                <div className={"youtube-thumbnail"} style={{
                                                                    ...
                                                                    <img
                                                                        src={`https://img.youtube.com/vi/${d.info.youtube.link.split("?v=")[1]}/hqdefault.jpg`}
                                                                        style={{
                                                                            width: "100%",
                                                                            height: "100%",
                                                                            borderRadius: 4,
                                                                ...
                                                                <img
                                                                    src={"https://opgg-desktop-data.akamaized.net/assets/images/icon-video.svg"}
                                                                    width={48}
                                                                    height={48}
                                                                    style={{
                                                                        position: "absolute"
                                                                    ...
                                                                    <>
                                                                        <div className={"chess-row"}>
                                                                            {Array.from(Array(7).keys()).map((k) => {
                                                                                let c = _.find(d.info.buildboards[time], {xy: `${i}-${k}`});
                                                                                return (
                                                                                    <div className={`hex ${c && `hex-${c.cost}`}`}
                                                                                         style={{
                                            ...
                                            <div className="tft-guide-modal-layer-info-youtube-channel" style={{
                                                    ...
                                                    <a href={youtube.channelUrl} target={"_blank"} onClick={() => {
                                                        ...
                                                        <div className="tft-guide-modal-layer-info-youtube-channel-info">
                                                            <div><img style={{
                                                                ...
                                                                borderRadius: "50%",
                                                                marginRight: 16
                                                            }} src={d.youtube?.thumbnailUrl}/></div>
                                                ...
                                                {twitch &&
                                                    <a href={twitch.channelUrl} target={"_blank"} onClick={() => {
                                                        ...
                                                        <div
                                                            className="tft-guide-modal-layer-info-youtube-channel-twitch btn-channel-twitch" style={{
                                                                marginRight: 16
                                                        }}>
                                                            ...
                                                            <div className={"tft-champion"}>
                                                                {c?.three &&
                                                                    <div style={{
                                                                        ...
                                                                        <img
                                                                            src={`https://desktop-app-data.op.gg/assets/images/star${c.cost}.svg`}/>
                                                                    </div>
                                                                }
                                                                <Tippy content={championToolTip(c)}
                                                                       disabled={championTooltipDisabled}
        ...
        return (
            <div className={`tft-guide-modal${isGuideOpen ? " tft-guide-modal__active" : ""}`} onClick={onRequestClose}>
                <div className="tft-guide-modal-layer" onClick={(e) => {
                    ...
                    {(localStorage.getItem("i18n") === "kr" ? guideInfo.info.text.kr : guideInfo.info.text.en) &&
                    <div className="tft-guide-modal-layer-text">
                        <h1>{t("tft-guide")}</h1>
                        <div id={"guide-text"}>{localStorage.getItem("i18n") === "kr" ? guideInfo.info.text.kr : guideInfo.info.text.en}</div>
                    </div>
                    }
                        ...
                        <div style={{
                            ...
                            <div className="tft-guide-modal-layer-info-hextech" id={"tft-hexcore"}>
                                <div style={{
                                    ...
                                    {guideInfo.info.hexcores.map((hexcore: any) => {
                                        if (hexcore.id !== 0) {
                                            ...
                                            return (
                                                <div className={"tft-guide-modal-layer-info-hextech-container"}>
                                                    <div className={"tft-guide-modal-layer-info-hextech-img"}>
                                                        <img
                                                            src={`https://desktop-app-data.op.gg/tft/${tftSet}/hexcores/${hexcore.id}.png`}/>
                                                    </div>
