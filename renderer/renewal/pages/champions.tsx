...
const Champions = ({champion}: any) => {
  ...
  const [liveOP, setLiveOP] = useState([]);
  ...
  const [onboarding, setOnboarding] = useState((localStorage.getItem("onboarding") ?? true) !== "false");
  ...
  useEffect(() => {
    ...
    if (_location.state) {
      ...
      try {
        let bla = _.filter(_.filter(champion.liveOP, {
          position: tmpPositionMap[champion.assignedPosition],
          subType: 420,
          tierRank: tmpTierMap[champion.soloRankTier.substring(0, 2)]
        }), (v) => {
          return v.rank >= 1 && v.rank <= 4;
        });
        if (champion.queueId === -1 || champion.queueId === 420) {
          setLiveOP(bla);
          let tmpLiveOP = [];
          bla.map((b, i) => {
            if (i < 4) {
              tmpLiveOP.push(b.championId);
            }
          });
          localStorage.setItem("liveop-champions", JSON.stringify(tmpLiveOP));
        }
      } catch (_) {}
      setPerkPage(0);
      setPerkCategory(0);
      ...
      try {
        ...
        localStorage.setItem("current-champion", champion.data.summary.id);
      } catch (e) {
      }
    ...
    return (
      <div className="main-container contents-container">
          ...
          {combo && !isOverlay &&
              ...
              >
                  <Tippy
                      ...
                      showOnCreate={true}
                      content={<div
                    dangerouslySetInnerHTML={{__html: t(`combo-growth`)}}/>}>
                      <div className="champion-button-combo-video">
                          <img src={"../../assets/images/img-sidebar-combo.svg"}
                               style={{
                                 width: "16px",
                                 marginRight: "4px"
        ...
        {tips && <LaneTip queueId={queueId} laneTipOpened={laneTipOpened} setLaneTipOpened={setLaneTipOpened}
                          tips={tips[lane]} championId={data.summary.id} tip={tip} setTip={setTip}
        ...
        <div className="champion-contents" style={{marginTop: `${tips ? "" : "52px"}`}}>
          ...
          <div className="champion-refresh" onClick={refresh}>
            <img
          ...
          <div className="champion-contents-left">
            <div className="champion-contents-left-top">
              <div className="champion-title">
                <div className="champion-title-name"
                     onClick={onClickChampionName(metaData?.key, lane)}>{t(`champions.${data?.summary.id}`)}</div>
                        ...
                        <>
                            <div className="champion-contents-container-row">
                                ...
                                <Tippy placement="bottom" content={<VideoTooltip isHover={videoTooltipHoverId === "R"}
                                                                                 championId={data.summary.id}
                                                                                 skillId={"R"}/>}>
                                    <div className="skills skills__R_main"
                              ...
                              {data?.skill_masteries[0].ids.map((skill, index) => {
                                return (
                                  <>
                                    ...
                                    {data?.summary.id === 77
                                        ? <>
                                          {index < 3 && <img className="arr-right"
                            ...
                            <div style={{display: "flex", flexDirection: "row"}}>
                              <div>{t("live.feature.champion.win-rate")}
                                <span style={{
                                  ...
                                  textAlign: "right"
                                }}>{(items.win / items.play * 100).toFixed(2)}%</span> {t("live.feature.champion.pick-rate")}
                                <span style={{
                                  width: "40px",
                                  display: "inline-block",
                                  textAlign: "right"
                                }}>{(items.pick_rate * 100).toFixed(2)}%</span></div>
                              <div><span style={{
                                width: "40px",
                                display: "inline-block",
                                textAlign: "right",
                                ...
                                fontWeight: "normal",
                                marginLeft: 0
                              }}>{formatter.format(items.play)}</span> Matches
                              </div>
                            </div>
                          </div>
                        </>
                                  ...
                                  <>
                                      ...
                                      <div
                                        ...
                                        {t("live.feature.champion.export-rune")}
                                          <img
                                              src={`../../assets/images/icon-check${currentPerkPage === index * 2 + i ? "-disabled" : ""}.svg`}
        ...
        <div style={{
          ...
          {queueId !== 450 && queueId !== 900 && counters && counters.length !== 0 &&
              <div className={`champion-counters ${liveOP.length > 0 ? "" : "champion-counters-full"}`}>
                      ...
                      if (index < cnt) {
                        return (
                          <div className={"champion-counters-item"}>
                            ...
                            <div className={"champion-counters-stats"}>
                              <div>W/Ratio</div>
                              <div style={{marginTop: 2}}><span>{((counter.WR) * 100).toFixed(2)}%</span></div>
                            </div>
                            {/*<div className={"champion-counters-seperator"}></div>*/}
                          </div>
                        )
          ...
          {liveOP.length > 0
              ? <div className={`champion-counters champion-liveop ${onboarding ? "onboarding" : ""}`} style={{
                marginLeft: `${counters?.length !== 0 ? "4px" : "0"}`
              }} onMouseEnter={() => {
                setOnboarding(false);
                localStorage.setItem("onboarding", "false");
              }}>
                  <div className="champion-counters-title" style={{display: "flex"}}>
                    <Tippy
                        className={`${onboarding ? "tippy-onboarding" : ""}`}
                        offset={[-20, 10]}
                        maxWidth={"270px"}
                        placement={"top-start"}
                        showOnCreate={true}
                        content={<div
                            dangerouslySetInnerHTML={{__html: t(`live.feature.champion.champion-recommend-tooltip`)}}/>}>
                      <div style={{display: "flex"}}>
                        {t("live.feature.champion.champion-recommend")}
                        <div style={{
                          width: 4,
                          height: 4,
                          borderRadius: 2,
                    ...
                    <div style={{marginLeft: "auto", fontSize: 11, color: "#5d5a73"}}>
                      {t(`live.feature.champion.champion-recommend-tip`)}
                    </div>
                  </div>
                <div className={"champion-counters-row"}>
                  {liveOP && liveOP.map((counter, index) => {
                    if (index < 4) {
                      return (
                          <div className={"champion-counters-item"}>
                            <div className={"img-wrapper"}>
                              <img
                                  src={`${_.find(championsMetaData.data, {id: counter.championId})?.image_url}?image=c_scale,q_auto,w_38`}/>
                            </div>
                            <div className={"champion-counters-stats"}>
                              <div>W/Ratio</div>
                              <div style={{marginTop: 2}}><span>{((counter.WR) * 100).toFixed(2)}%</span></div>
